...
onClickOutside(multiselect, () => {
    useDropdownStore().closeAssignControllerDropdown()
})
</script>
    ...
    <div ref="multiselect" class="w-full my-2">
        <div class="relative">
            ...
            <input type="text" v-model="searchService" v-on:keyup="getSearchResult(options)"
                ...
                placeholder="Назоратчи исмини киритинг..." />
        </div>
        ...
        <div class="w-full my-2 overflow-auto divide-y divide-gray-100 max-h-60" :id="id">
            <div v-if="!searchService" v-for="(option, idx) in options" :key="idx"
                @click="useMultipleSelectStore().setSelectController(option)"
                ...
                <CheckCircleOutlineIcon v-if="useMultipleSelectStore().selectedControllers.includes(option)"
            ...
            <div v-if="searchService && searchResults?.length === 0"
                ...
                <p class="text-red-500">Назоратчилар мавжуд эмас!</p>
            </div>
        </div>
    </div>
</template>
